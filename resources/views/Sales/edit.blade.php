...
@endif
<div class="container">
    <h1>Edição da Venda</h1>
    <form method="POST" action="{{ route('sale.update',[ $sale->id]) }}" class="form-horizontal form-validate">
        {{csrf_field()}}
        @method('PUT')
        <div class="form-group">
            <label>Produto:</label>
            <input id="produto" class="form-control" name="produto" type="text" placeholder="Digite o nome do produto" value="{{old("produto","$sale->produto")}}" required/>
        </div>
        <div class="form-group">
            <label>Data:</label>
            <input id="data" class="form-control" name="data" type="date" placeholder="Digite a data da venda" value="{{old("data","$sale->data")}}" required />
        </div>
        <div class="form-group">
            <label>Valor:</label>
            <input id="valor" class="form-control" name="valor" type="number" placeholder="Digite o valor da venda" value="{{old("valor","$sale->valor")}}" required/>
        </div>
        <div class="form-group">
